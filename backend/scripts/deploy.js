...
async function main() {
    ...
    const daix = await sf.loadSuperToken("fDAIx") //argument must be changed accordingly to switch(load) between other supertokens.
    ...
    //     daix.address 
    // )
    const TokenSpreaderFactory = new ethers.ContractFactory(
        TokenSpreaderABI,
        TokenSpreaderJSON.bytecode,
        signer
    );

    const tokenSpreader = await TokenSpreaderFactory.deploy(
        daix.address 
    );
